...
d={(i,j,k): distance(tasks[i-1],tasks[j-1]) for i in range(0,number_of_tasks+1) for j in range(0,number_of_tasks+1) for k in range(1,len(employees)+1)}
...
t={i: tasks[i].TaskDuration for i in range(0,number_of_tasks+1)}
...
s_tasks={i: tasks[i].OpeningTime for i in range(0,number_of_tasks+1)}

e_tasks={i: tasks[i].ClosingTime for i in range(0,number_of_tasks+1)}

level_employee={i: employees[i-1].Level for i in range(1,len(employees)+1)}

level_task={i: tasks[i].Level for i in range(0,number_of_tasks+1)}
...
for task1 in range(1,number_of_tasks+1):
    Realisation_constr[task1]=m.addConstr(quicksum(DELTA[(task1,task2,employee)] 
                                        for task2 in range(0,number_of_tasks+1) 
...
for k in range(1,len(employees)+1):
    Start_employee[k]=m.addConstr(quicksum(DELTA[(0,j,k)]
                                    for j in range(0,number_of_tasks+1))==1)
                                    
End_employee=dict()
for k in range(1,len(employees)+1):
    End_employee[k]=m.addConstr(    quicksum(DELTA[(i,0,k)] 
                                    for i in range(0,number_of_tasks+1))==1)

Flow_constr=dict()
for k in range(1,len(employees)+1):
    for t in range(0,number_of_tasks+1):
        Flow_constr[(k,t)]=m.addConstr(quicksum(DELTA[(i,t,k)]
                                        for i in range(0,number_of_tasks+1))==
                                        quicksum(DELTA[(t,j,k)]
                                        for j in range(0,number_of_tasks+1)))

m.setObjective(quicksum(DELTA[(i,j,k)]*d[(i,j,k)] for i in range(0, number_of_tasks+1)
                                                for j in range(0,number_of_tasks+1)
