...
def readingData(country):
    ...
    skillToRank = {skill: i for i, skill in enumerate(
        set(employees_sheet["Skill"]))}
    employee_to_rank = {employee_name: i+1 for i,
                        employee_name in enumerate(list(employees_sheet["EmployeeName"]))}
    ...
    for index, row in employees_sheet.iterrows():
        ...
        employees.append(TEmployee(row["EmployeeName"], row["Latitude"], row["Longitude"],
                                   skill, row["Level"], workingStartTime, workingEndTime, []))

     # employees_unavailabilities
    employees_unavailabilities_sheet = pd.read_excel(
        xls, 'Employees Unavailabilities')

    for index, row in employees_unavailabilities_sheet.iterrows():
        Start = dateToMinute(row["Start"])
        End = dateToMinute(row["End"])
        unavailability = TUnavailability(
            row["Latitude"], row["Longitude"], Start, End)
        employees[employee_to_rank[row["EmployeeName"]]
                  ].Unavailabilities.append(unavailability)

    # tasks
    task_sheet = pd.read_excel(xls, 'Tasks')
    task_to_rank = {task: i+1 for i,
                    task in enumerate(list(task_sheet["TaskId"]))}
    ...
    for index, row in task_sheet.iterrows():
        ...
        tasks.append(TTask(row["TaskId"], row["Latitude"], row["Longitude"],
                           row["TaskDuration"], skill, row["Level"], openingTime, closingTime, [], 0, 0))
